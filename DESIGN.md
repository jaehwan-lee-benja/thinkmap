...
> 정본 성문화. 근거: `사르르목장_brand_manual.ai`(2021·2022) · `DESIGN-SYSTEM.md` · `docs/DESIGN-CRAFT-CANON.md`.
...
이 파일의 **정본은 `saruru-design/DESIGN.md` 하나**이고, 소유는 design 이다. 다른 곳의 같은 파일은 **사본**이다.

- **사본이 최신인지는 «받는 쪽»이 스스로 잰다** — `saruru-design/docs/toolkit/canon_check.py <내 repo 경로>`.
  통지를 기다리지 마라: **발신자 의존 규범에서 «침묵»은 «갱신 없음»과 «통지 없음» 둘 다와 양립한다.**
- ★**공개/비공개로 사본의 형태가 갈린다**(2026-08-17 판정 · 규범 충돌 해소):
  · **비공개 사본** = 이 파일 **실물 그대로** 복사.
  · **공개 사본** = **브랜드 값만** 담은 **투영본**. 서두에 「이 문서는 정본의 부분 투영이며, 여기 없는 절이 있어도
    표류가 아니다」를 반드시 적는다(없으면 **의도된 투영**과 **침묵의 표류**를 구별할 수 없다).
  · 이 정본은 **브랜드만 담는다** — 운영·토폴로지·사고 기록은 정본에 넣지 않는다(넣으면 위 두 규칙이 서로 당긴다).

운영 기록(미러 대장·표류 실측·회수 이력)은 design 내부 문서에 있다.
...
- ★**검증 규범 ⑶ 두 기전을 함께 쓴다** — 대비 모집단 = **⒜선언 규칙 순회 ∧ ⒝그 규칙의 실 타깃 증거**
  ...
  불일치가 사각의 위치를 지목한다**. — «A ∧ B, 어느 하나도 단독으로는 불충분» 계열의 규범이다.
...
> ※이 절은 **2026-08-16 신설**이다. 사본을 들고 있다면 「정본과 사본」 절의 자가측정을 돌려라 —
> 신설 다음 날 실측에서 **사본 3개가 이 절의 존재를 몰랐다.**

★**정본 = `docs/MOTION-CANON.md`** (지속시간·이징·동시성·금지·축하 어휘·접근성·QA 체크리스트). 화면 산출 전부(키오스크·대시보드·게임 위성·응원화면)에 적용. 아래는 요지만 — 상세는 정본을 따른다.

- **지속시간**: 피드백 100ms / 상태 변화 160ms / 패널 240ms / 화면 전환 300ms. ★**상한 400ms**(축하 연출만 예외).
- **이징**: 표준 `cubic-bezier(.4,0,.2,1)` · 등장 `cubic-bezier(0,0,.2,1)` · 퇴장 `cubic-bezier(.3,0,1,1)`. ★**등장은 종단속도 0**(감속 착지) — 현역 제품 3종 실측 수렴.
- **동시성**: 한 화면의 등장은 **동시에·1회**. 요소별 딜레이 체인 금지(예외 4조건은 정본 §5).
- **등장 기본형**: `opacity 0→1` + `scale .98→1`. **`translate` 등장(슬라이드 인) 금지.**
- **어르신 친화 ≠ 느린 모션**: 느림은 배려가 아니라 대기다. 속도가 아니라 **모션의 양**을 줄이고, 읽을 내용은 전환이 끝난 뒤 **머물게** 한다.
- **`prefers-reduced-motion` 분기 필수** — 모션 없이도 뜻이 전달되어야 한다.
...
- ★**모션**: 슬라이드 인(화면 밖에서 밀려 들어오는 등장)·요소별 딜레이 체인(순차 등장)·400ms 초과 UI 전환·바운스 과다(오버슈트 6%↑ 또는 되튐 2회↑)·로고 애니메이션. → 정본 `docs/MOTION-CANON.md` §6.
