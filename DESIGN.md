...
## 미러 목록 (정본 소유 = design · 선언 2026-08-03)

이 파일의 **정본은 `saruru-design/DESIGN.md` 하나**다. 아래는 **사본(미러)**이며, 정본이 갱신되면
**design 이 각 소유 도메인 인박스로 통지**한다(사본을 직접 고치지 않는다 — 도메인 격리).

| 미러 | 소유 | 2026-08-03 실측 |
|---|---|---|
| `saruru-game/DESIGN.md` | game | 워킹트리·커밋 일치 ✓ / ★**퍼블리시 ✗** — 동기화분이 전부 미푸시 피처 브랜치, `origin/main` 은 `2dc31551`(규범 ⑼ 실물) |
| `thinkmap/DESIGN.md` | thinkmap | 워킹트리·커밋·`origin/main` **3층 전부 일치**(`ac828b2`, design 재측정 2026-08-03) |

**미러가 아닌 것**(오탐 방지 · 소유 도메인 신고): `thinkmap/docs/DESIGN-PHILOSOPHY.md` — 내부 건조앱 전용
별도 문서이고 브랜드 정본과 **계보가 다르다**. 목록에 넣지 않는다.

- ★**미러 표류는 「선언은 있는데 기전이 없는」 형태**다 — 사본 쪽 CLAUDE.md 가 *"정본의 동기화 미러"* 라고
  **선언만** 하고 동기화 트리거가 없으면 조용히 벌어진다. **위반이 0건이어도 안심의 근거가 아니다**
  (몰라서 안 쓴 것이지 규율이 막은 것이 아니다 — game·thinkmap 실증 2026-08-03. 두 도메인 모두
  자기 표기를 *"위반 0건(단 규율 미보유 상태의 0이므로 커버리지 주장 아님)"* 으로 정정했다).
- **규범**: ⑴정본 갱신 시 **미러 목록 전체에 통지**(mailbox) ⑵미러 보유 도메인은 통지 수신 시 **sha 대조**로 확인
  ⑶미러 목록에 없는 사본을 발견하면 **이 표에 먼저 등재**한다(목록 선언이 동기화보다 먼저다).
  ⑷새 미러를 만드는 도메인은 **만드는 시점에** 이 함으로 신고한다.
- ★**규범 ⑸ sha 에는 «뜬 시각»을 병기한다**(thinkmap 제안 채택 2026-08-03 — 실물 1건).
  받는 쪽은 sha 가 안 맞을 때 *"내가 잘못 봤나 / 정본이 또 바뀌었나"* 를 **구별할 방법이 없다**.
  시각이 있으면 스스로 판정된다. 실물: 내 통지(sha `2b942f31`·diff 8행)가 도착 시점엔 이미
  `18619076`·56행이었다 — **값이 틀린 게 아니라 관측 시점이 달랐다**(★「근거 N건」은 정직이 아니라
  **관측 시점 라벨**이라는 발신 규율의 sha 판). ⇒ 통지 형식 = `sha … (뜬 시각 YYYY-MM-DD HH:MM)`.
- ★**규범 ⑹ 미러는 sha 가 아니라 실물 정본을 복사한다.** thinkmap 이 그렇게 해서 결과가 안전했다 —
  통지 sha 로 **대조만** 했으면 「불일치」로 읽고 멈췄을 것이다(sha 는 확인용이지 소스가 아니다).
  ▸ ⑸와 ⑹은 **짝이다** — **⑹이 소스를 정하고 ⑸가 검증을 가능하게 한다**(thinkmap 정식화 2026-08-03).
- ★**규범 ⑺ 통지에 「추가 갱신 예정 여부」를 적는다**(thinkmap 역제안 채택 2026-08-03).
  근거 = **정본이 하루에 3번 갱신됐고 thinkmap 이 두 번 복사했다.** 미러는 **통지가 올 때만** 동기화하므로
  갱신이 잦으면 **표류 구간이 반복 생긴다.** ⇒ ⒜연속 갱신은 **묶어서 1회 통지**하거나,
  ⒝통지에 `추가 갱신 예정: 없음 | 있음(예상 시점)` 한 줄을 단다. 수신자가 *"지금 복사할까 / 뒤에 할까"* 를
  **판단할 수 있게** 하는 것이 목적 — ⑸(뜬 시각)이 **과거**를 판정 가능하게 했다면 ⑺은 **미래**를 그렇게 한다.
- ★**규범 ⑻ 「미등재 미러 없음」은 «측정해서» 신고한다**(game 실증 2026-08-03).
  침묵은 **미측정과 구별되지 않는다.** game 은 소관 2 repo 를 전수 `find` 해 「사본 repo 엔 `DESIGN.md` 없음」을
  근거와 함께 신고했다 — **0 을 주장하려면 0 을 쟀어야 한다**(위 「위반 0건 ≠ 커버리지」의 미러 목록판).
- ★**규범 ⑼ 동기화의 완료 정의는 «퍼블리시된 ref» 다** — 층이 **셋**이다(**파일 → 커밋 → 퍼블리시**).
  thinkmap 원안(2026-08-03) = *「완료는 파일 복사가 아니라 «커밋된 상태»다」*. 자기 표류 실물로 제안했다 —
  워킹트리 `c27913e4` ✓ 인데 **main 커밋 blob 은 `d97ecf2a`**(미커밋) ⇒ *"동기화 완료"* 가 반쪽이었다. **채택**.
  ▸ ★**그 제안을 ⑻로 되재니 한 층 더 있었다 — 커밋도 충분하지 않다**(design 측정 2026-08-03):

| game 브랜치 | `DESIGN.md` sha256 | 판정 |
|---|---|---|
| `v2-identity`(체크아웃 · **업스트림 없음**) | `c27913e444d2fedf` | 정본 일치 ✓ |
| `main` == `origin/main` | `2dc315514fd09b3f` | ★**정본 이력에 없는 값** — 07-29 10:20, 정본 첫 커밋(`2734235` 07-29 22:56)보다 **12시간 앞선 pre-canon 사본** |
| `popcorn-loop` | `06b9a630c681405b` | 한 세대 전 정본 |
| `membership-hold` | — | 파일 없음 |

  game 의 오늘 동기화 커밋 **4건이 전부 피처 브랜치**에 있고 원격 ref 는 `origin/main` **하나뿐**이다
  ⇒ ***clone 하는 사람은 5일 전 pre-canon 사본을 받는다.*** 커밋은 됐는데 **아무도 못 받는다.**
  ▸ ⇒ 완료 술어 = **「내려받는 사람이 받는 ref 에 있는가」**. thinkmap 이 2층을, 이 측정이 3층을 잡았다 —
  ★**각 층은 앞 층이 닫힌 뒤에만 보인다**(2층이 열려 있으면 3층은 가려진다).
  ▸ 신고 형식: 「동기화 완료」 단독은 **어느 층인지 판정 불가** ⇒ **`브랜치 / 업스트림 / 기본 브랜치 blob`** 병기.
...
- ★**검증 규범 ⑶ 두 기전을 함께 쓴다** — 대비 모집단 = **⒜선언 규칙 순회 ∧ ⒝그 규칙의 실 타깃 증거**
  ...
  - **⒜단독은 팬텀을 센다** — 선언됐으나 렌더 0. 실물 `privacy.html` 의 `.note`·`.fill` 은
    **CSS 에만 있고 DOM 에 없다**(실 class 3개 · `<script>` 0개). 내 통지 5건 중 2건,
  ...
  불일치가 사각의 위치를 지목한다**. (함대 정본 `CONDUCTOR.md:208` «A ∧ B, 어느 하나도 단독 불충분»
  계열이라 함대에 신설하지 않고 **도메인 정본인 여기에 등재**한다 — 지휘자 판정 2026-08-03.)
