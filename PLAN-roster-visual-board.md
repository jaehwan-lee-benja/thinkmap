...
- **Phase C — 드래그 배치**: dnd-kit 멤버 풀↔슬롯, 슬롯 간 이동(role/shift 변경).
- **Phase D — 레이아웃 편집 + 버전 저장(사이트 내)**: 슬롯 추가/삭제/드래그 이동·세부업무 편집,
  "이 체제 갱신" / "다른 이름으로 저장"(신규 버전).
- **Phase E — 출력/풀스크린 읽기 뷰** + 인쇄 CSS.
