...
- **P6. 보드 데일리는 보드 소유다.** 한 보드의 하루치 페이지는 전 멤버가 공유하는 **보드의 것**이다. 읽기/편집 권한은 `pages.user_id`(만든 사람)가 아니라 **보드 멤버십 + 역할**(`worklog_board_members`)에서 파생된다. `user_id`는 `created_by` 감사용 메타데이터로 강등한다. (현재 RLS는 "만든 사람" 기준이라 보드 멤버십을 안 쓰는 갭이 있음 — §5 Phase에서 정렬.)
  - **L1 테넌시 선행 작업**: 이 "멤버십 + 역할에서 파생" 원칙과 `worklog_board_members` / `is_board_member_of_page`(Phase 0.7)는 보드 한정 기능이 아니라, 향후 **workspace / members / roles 테넌시 계층**의 씨앗이다. 멤버십을 확장할 때는 "보드 한정"이 아니라 "테넌시의 일부"로 설계한다. → [docs/ACCESS-MODEL.md §6](docs/ACCESS-MODEL.md).
