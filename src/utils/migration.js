 ...
 * 마이그레이션 검증 함수
 */
export async function validateMigration(userId) {
  console.log('\n🔍 마이그레이션 검증 시작...')
 ...
 */
export async function migrateJSONtoBlocks(userId) {
  console.log('\n🚀 마이그레이션 시작...')
  ...
  try {
    ...
    const traverse = (blockList, parentId = null) => {
      blockList.forEach((block) => {
        ...
        if (Array.isArray(block.children) && block.children.length > 0) {
          traverse(block.children, blockId)
        }
      })
    }
    ...
    if (isValid) {
      ...
      return { success: true, count: flattenedBlocks.length }
    } else {
      console.error('\n⚠️  마이그레이션은 완료되었으나 검증에서 문제가 발견되었습니다.')
      console.error('   rollbackToJSON() 함수를 실행하여 복구할 수 있습니다.')
      return { success: false, count: flattenedBlocks.length }
    }
  } catch (error) {
 ...
 */
export async function rollbackToJSON(userId) {
  console.log('\n🔄 롤백 시작...')
  console.log(`User ID: ${userId}`)
