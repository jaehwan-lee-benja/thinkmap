...
export default function SeatSystemPage({ session }) {
  ...
  return (
    <div className="seat-app">
      ...
      <main className="seat-main">
        {role.key === 'guide' ? (
          <GuideScreen orders={orders} stations={stations} onPatch={onPatch} onCreate={onCreate} />
        ) : role.key === 'manager' ? (
          <ManagerScreen role={role} orders={orders} stations={stations} onPatch={onPatch} onCreate={onCreate} />
        ) : role.station ? (
