...
function BackupModal({
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} className="backup-modal">
      <ModalHeader icon={HardDrive} title="프로젝트 백업" onClose={onClose} />
      ...
      <div className="backup-create-section">
        <div className="backup-create-form">
          <input
            ...
            disabled={isLoading}
          />
          <button className="backup-create-button" onClick={handleCreateBackup} disabled={isLoading}>
            <Plus size={18} />
            <span>백업 생성</span>
          </button>
        </div>
        <button className="backup-import-button" onClick={handleImport} disabled={isLoading}>
          <Upload size={16} />
          <span>파일에서 가져오기</span>
        </button>
      </div>
                ...
                <div className="backup-item-content">
                  <div className="backup-item-header">
                    ...
                    <div className="backup-item-actions">
                      <button className="backup-action-btn" onClick={(e) => handleExport(backup, e)} title="파일로 내보내기">
                        <Download size={14} />
                      </button>
                      <button className="backup-action-btn delete" onClick={(e) => handleDelete(backup, e)} title="삭제">
                        <Trash2 size={14} />
                      </button>
                    </div>
                  </div>
                  <div className="backup-item-description">{backup.description}</div>
                  <div className="backup-item-meta">
                    <span>{formatFullTime(backup.createdAt)}</span>
                    <span>{backup.pages?.length || 0}개 페이지</span>
                  </div>
                  <button className="backup-restore-button" onClick={() => handleRestore(backup)} disabled={isLoading}>
                    <RotateCcw size={14} />
                    <span>이 시점으로 복원</span>
                  </button>
                </div>
