-- blocks 테이블 생성: 개별 레코드 방식으로 블록 저장
-- parent_id + position으로 계층 구조 관리
-- is_reference + original_block_id로 블록 참조(Synced Block) 기능 지원

-- 1. updated_at 자동 갱신 함수 생성 (트리거용)
...
-- ====================================================================
-- block_history 테이블 생성: 블록별 수정 이력 추적
-- ====================================================================
...
DO $$
BEGIN
  RAISE NOTICE '✅ blocks 테이블 및 block_history 테이블 생성 완료';
  RAISE NOTICE '- blocks: parent_id + position 계층 구조';
  RAISE NOTICE '- blocks: is_reference + original_block_id 참조 기능';
  RAISE NOTICE '- block_history: 블록별 수정 이력';
  RAISE NOTICE '- 인덱스: 검색, 계층, 참조 최적화';
