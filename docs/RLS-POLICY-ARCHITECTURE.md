...
### 1-x. ★Edge Function 인증 — `verify_jwt` 는 «꺼야» 한다 (2026-08-15 앵커)

브라우저에서 부르는 Edge Function 은 **`verify_jwt = false`** 로 등재하고 **함수 내부에서 `getUser()` 로 검증**한다.
보안 완화가 아니라 **CORS 프리플라이트** 때문이다:

> **OPTIONS 프리플라이트 요청에는 `Authorization` 헤더가 없다.**
> 게이트웨이 검증(`verify_jwt = true`)을 켜면 그 프리플라이트가 **401 로 거부**되고,
> 브라우저는 본 요청을 **보내지도 못한다** — 화면에서 호출 자체가 불가능해진다.

· **강도는 같다.** 검증 «위치»만 게이트웨이 → 함수 안으로 옮기는 것이고, 익명 호출은 함수가 `401` 로 거부한다.
· 레포 전수 일치: `ensure-daily-page` · `membership-*`(6종) · `kiosk` · `spend-queue` · `spend-verdicts` **전부 false**.
  (`config.toml:416~417` 이 이미 이 사유를 적고 있었다.)
· ★**이 항목이 정본에 없어서 2026-08-15 계약 초안이 «true» 로 확정될 뻔했다** — 원칙(「층이 하나 더 낫다」)이
  이 레포의 사실을 이겼던 사례다. 그래서 여기 앵커한다.
· 함수 내부 규범: `Authorization` 헤더를 **그대로 실어** anon key 클라이언트를 만들고(`global.headers`),
  `getUser()` 로 신원을, 그다음 **DB 함수 한 개**로 권한을 묻는다. **Edge 가 자체 권한 판정을 하지 않는다** —
  판정이 두 곳이면 반드시 어긋난다.
