...
- ⒜ **BT 프린터 또는 BT-USB 어댑터 도입** → §4 가 그날 바로 유효해진다.
  ...
  ★**교차 근거(2026-08-10)**: 사진-영수증 기능 타당성 조사(`~/claude-project/docs/KIOSK-PHOTO-RECEIPT-FEASIBILITY.md` §39)가
  **독립적으로 같은 하드웨어 결론**에 도달했다 — 외장 USB 카메라와 프린터를 USB 로 동시에 물리면
  powered OTG 허브 + 전력 경합이라, **프린터를 블루투스로 빼는 구성을 권장**한다.
  ⇒ 그 기능이 착수되면 «BT 프린터»는 **그 기능의 비용**으로 이미 지불되고, 이 문서의 ⒜ 는 **덤으로 열린다**
  (인쇄 결과를 알 수 있게 되고 컷 주체가 하나로 확정된다). 두 사안이 **하드웨어 결정 하나를 공유**한다 —
  BT 전환의 손익을 볼 때 이쪽 이득을 빼놓고 계산하지 마라.
