...
- 문구 = 같은 파일의 `emptyText(state, readyText)`. ★**빈 자리 문구는 전부 이걸 거친다.**
  ...
  셋이 빠져 있었다(2026-08-17 D 트랙 적발). 읽기 실패 때 그 화면은 여전히 «고장을 정상 얼굴로» 착지시켰다.
...
- `apps/seat/src/components/Seat/utils/emptyStateAudit.{js,test.js}` — **「빈 자리 문구는 전부 emptyText 를 거친다」에 붙인 기계**(2026-08-17).
