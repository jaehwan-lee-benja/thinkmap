...
- ★**design 판정 도착(2026-08-17, orch 경유)** — 정본이 **개정**됐다(§7-1 신설: 축하 «입자·물리»는 §6-3 바운스 한도 면제):
  ...
  · 오버슈트·되튐 = **면제 대상**. ⚠단 면제는 **경계 조건 6개를 전부 지킬 때만**이다. 실측 = **4/6 통과**:
    ⓐ총 ≤2.0s ✓(.7~.85s) · ⓑ1회·재트리거 금지 ✓(`celebrating` Set + 버튼 `disabled`) · ⓔ완전 정지 ✓(루프 0)
    · ⓕreduced-motion 렌더 안 함 ✓(`animation:none` → 입자의 초기 `opacity:0` 유지)
    · ⓒ**스킵 가능 ✗** — 700ms 동안 건너뛸 수단이 없다.
    · ⓓ**입력을 막지 않는다 ✗** — `StationScreen.complete()` 가 **완료 쓰기 자체를 `setTimeout(…, 700)` 뒤로 미룬다.**
      애니메이션이 **임계 경로**에 있다 = 주방에서 «완료를 눌렀는데 0.7초 뒤에 반영»된다.
    ⇒ 그래서 이 두 줄은 **아직 명단에 남는다**(면제 조건 미충족). 고치려면 «쓰기는 즉시 · 연출은 별도»로 분리해야 하는데
      그건 카드가 목록에서 사라지는 시점을 바꾼다 = **또 결정**이라 내가 단독으로 안 한다.
