...
- ★**[안내중] ↔ [완료] 탭**(2026-08-08, R12) — 표 위 토글 2개(건수 뱃지). 기본 = **안내중**.
  - 행 오른쪽 **`✓`**(삭제 ✕ 왼쪽) = **안내 완료 → 완료 리스트로 아카이빙**(`archived_at=now()`).
  - **완료 탭**에서는 같은 자리가 **`↩`**(대기열로 복귀, `archived_at=null`). **되돌릴 수 있는 상태 전환이라 재확인 모달 없이 즉시.**
