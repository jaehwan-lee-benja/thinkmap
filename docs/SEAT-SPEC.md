...
- ★**[안내중] ↔ [완료] 탭**(2026-08-08, R12) — 표 위 **탭 바**(건수 뱃지). 기본 = **안내중**.
  ...
  - ★★**규칙을 코드로 강제한다**(2026-08-09 리팩토링 ⑶ — 위 세 함정이 하루에 3연속으로 났고 원인이 전부 같은 종류였다):
    - **스크롤 상자는 `.seat-scrollport` 하나뿐**이다(`.seat-main` 이 이 클래스를 단다). 규칙(overflow·overscroll·**패딩 0**)이
    ...
    - **dev 가드**(`utils/seatDevGuard.js`, 프로덕션 무영향) — 스크롤포트가 둘이 되거나 / 스크롤포트에 패딩이 붙거나 /
...
| `raiseMethodOf(o)` | 올림취소 이력(R10) | ★우선순위가 `optOf` 와 다르다(**포장 먼저**) — 구 동작 보존, 아래 주 참조 |
| `optOf(o)` / `optPatch(v)` | 제조옵션 드롭다운 | boolean 3개는 **실제로 단일 선택** — 여기를 통하면 «둘이 켜진» 상태를 만들 수 없다 |
| `isParallel(o)` | 완료 버튼 3색 | 야외병행 판정(§9.0) |

※`raiseMethodOf` 의 우선순위(포장→야외→야외병행)는 `optOf`(야외→포장→야외병행)와 **다르다**. 구 코드가 두 곳에서
  서로 다른 순서를 쓰고 있었고, 두 컬럼이 동시에 `true` 인 행이 있으면 취소 이력 라벨이 갈린다. 현재 쓰기 경로는
  전부 `optPatch` 를 통과해 «하나만 true» 를 보장하므로 **UI 로는 도달할 수 없는 조합**이지만, 구 데이터 유무를 확인할 수
  없어 **구 동작을 그대로 보존**했다(기능 변경 0). 통일하려면 실데이터 확인이 먼저다.
