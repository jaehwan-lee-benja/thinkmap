# 업무일지 기획서 v1 (WorkLog Specification — Archived)

> ⚠️ **이 문서는 보존용 이전 버전(v1)입니다.** 현행 명세는 [WORKLOG-SPEC.md](./WORKLOG-SPEC.md) (v2 — JSON → 개별 row 리팩토링 기획서) 를 참조하세요.
>
> v1 → v2 핵심 변경: daily 페이지 본문을 `pages.content_tiptap` JSONB 한 덩어리에 저장하던 모델에서, 개별 todo/section 을 별도 테이블의 row 로 분리하는 데이터 모델 전환.
>
> 작성일: 2026-04-12
> 최종 업데이트: 2026-04-23
> 백업일 (아카이브 표시 추가): 2026-04-28
> 상태: Phase 1~6 완료 (리팩토링 v2 출범 전 마지막 스냅샷)
