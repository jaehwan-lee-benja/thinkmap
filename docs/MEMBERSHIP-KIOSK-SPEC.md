...
④ 보안 : 매장 태블릿 = 최소권한. 정확한 전체 번호 매칭만(부분검색·목록 금지)+레이트리밋+직원게이트.
          고객모드(180° 가입)는 조회결과 안 보이는 제한 서브뷰.
```
...
| 6 | 쉬운 회원가입(180° 고객모드) | 고객모드 가입폼 → crm `membership-intake` | v1 |
...
- 입력: `{ phone, name, consent:true, source:"kiosk" }` — 최소 필드.
...
├─────────────────────────────────────────────────────────────────────┤
│  고객 가입모드 (화면 180° 회전, 고객이 직접 입력)                     │
│   전화번호 [ 대형 패드 ]  이름 [    ]  [ ] 개인정보 동의  → [가입]     │
...
- **모드 전환**: 직원모드 기본. 고객모드 진입 시 조회 UI 숨김·가입폼만(180° 회전 CSS). 복귀는 직원 확인(간단 탭 or PIN, §5).
...
5. **고객모드 격리**: 180° 가입모드는 **읽기 없이 쓰기(가입 write)만** — 조회 결과가 화면에 없음(고객이 태블릿 만져도 타인 정보 안 보임). 직원모드 복귀에 경량 게이트(직원 탭/PIN) 권장.
...
- **고객 가입모드 = 읽기 없이 쓰기만**(신규 가입 write). 180° 돌려 고객이 직접 입력해도 남의 회원정보 못 봄.
