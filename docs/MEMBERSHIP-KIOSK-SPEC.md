...
### 5.0 ★브랜드 보이스 — 카피 기준 (유저 확정 2026-08-06)
> 유저 원문: 「'팝콘 1개 더!' 이건 **체리피커처럼 되어서, 단순히 팝콘을 받기 위한 여정**이 돼.
> 지금은 **사르르목장을 더 즐길 수 있는 가이드, 안내자가 되는 컨셉**이 더 중요해.
> **체리피커는 오히려 거를 수 있는 과정이 더 좋아.**」

- **금지**: 보상·수치 미끼(«N점 넘기면 팝콘 1개 더», «지금 가입하면 …» 류). **혜택을 미끼로 걸지 않는다.**
- **채택**: 경험·안내 소구 — 우리는 **판매자가 아니라 안내자**다. 예: `사르르목장에 숨은 게임을 경험해보세요!`
- 근거: 미끼 카피는 **체리피커를 부르고**, 그건 우리가 원하는 고객군이 아니다(거르는 편이 낫다).
- 적용 범위: 영수증·키오스크 화면·티켓 화면 등 **고객이 보는 모든 문구**. 새 카피를 쓸 때 이 절을 먼저 본다.
- ※ 관련 선례: 브랜딩 금지어(«당일착유·오늘짠우유») — 같은 계열의 «유저가 직접 정한 보이스 기준»이다.

### 5.3 ★손님 폰 «감사» 화면 신호 — 잔여 리스크 (2026-08-08)

회수 확정을 손님 폰(`?role=ticket`, 계정 없음)에 알리는 Realtime 룸 `membership-ticket` 은
**수신=anon / 발신=직원 세션만** 의 비대칭 인가다(`migrate-membership-ticket-thanks-realtime.sql`).

- payload = `{ token, stamp:'n/10' }` — **이름 없음**. token 은 **회수 후에만** 나가므로 이미 소진(재사용 가치 0).
  ★역으로 «발권 시점»을 이 룸에 쏘면 **미소진 토큰이 샌다 — 절대 금지**.
- ⚠**잔여 리스크(수용)**: 이 룸은 **매장 스코프가 없는 단일 전역 룸**이다(QR 페이로드에 store 가 없어 폰이 룸을 모른다).
  ⇒ 룸 이름을 아는 쪽은 **회수 이벤트의 빈도·타이밍·도장 분포**를 계속 엿들을 수 있다. PII 는 아니지만
  **경미한 영업정보**다(방문 빈도). 단일 매장 범위에서 수용하되, **다매장 확장 전에 store 스코프화**한다
  (`membership:<store>` 계열과 같은 축 — ACCESS/realtime 확장 시 함께).
...
**★예외 1건 — `?role=ticket`(손님 폰 티켓 표시)은 로그인 게이트 밖이다** (2026-08-03 등재)
- **무엇**: 키오스크 발권 화면의 QR을 손님이 자기 폰으로 찍으면 열리는 화면. 이미 발권된 티켓의 **바코드를 표시만** 한다.
- **왜 예외인가**: 이 화면을 보는 사람은 **고객**이라 매장 계정이 없다 — 게이트 뒤에 두면 **원리적으로 못 연다**.
- **왜 안전한가**(이 절의 취지를 침해하지 않는 이유): 이 화면은 **서버를 한 번도 부르지 않는다.** 데이터는 URL **프래그먼트(`#`)에 자족적으로** 들어 있고(프래그먼트는 서버로 전송되지 않는다) **조회·쓰기가 0**이다.
  ⇒ 이 절이 막으려는 것(**전 고객DB 노출·익명 조회 오라클**)이 **생성되지 않는다.** 담기는 PII 는 **마스킹 표시명**뿐이고, 노출 범위는 **종이 영수증과 동일**(그 순간 화면을 본 사람만)이다.
- **불변**: **회수(redeem)는 여전히 직원 게이트 뒤에서만** 일어나고 1회성·당일 만료도 서버 판정이다. 즉 이 화면은 **권한을 주지 않는다**.
- **알려진 위협(수용)**: 페이로드의 `name`·`date` 는 클라이언트 값이라 **위조된 URL로 엉뚱한 이름을 띄우는 "표시 사기"**가 가능하다. 그러나 **재화 이득 경로가 아니다** — 적립·수령은 토큰 기준 서버 판정이라 영향 0. 심각도 낮음으로 **수용**하고 여기 기록한다.
- ※ `?role=printer`(카운터 폰)·`?role=scan`·`?role=staff`·`?role=editor` 는 **예외가 아니다** — 전부 이 게이트 뒤에 있다.
- ★**교정 2026-08-16 — 인쇄 토폴로지**: 영수증 프린터는 **키오스크 유선 직결**이다. `?role=printer`(카운터 폰
  인쇄 브리지)는 **옛 토폴로지**이며 지금은 **대비 경로**로만 남긴다(폐기 아님 — 직결 고장 시 되돌아갈 길).
  ★옛 전제를 지우지 않는 이유: 이력이 없으면 다음 사람이 또 «폰 경유»로 설계한다. 실제로 그렇게 만들어진
  브리지가 이미 있고, 지금은 꺼져 있다(`VITE_MEMBERSHIP_REALTIME` 미설정). 켜기 판정은 «실출력 시험 1장» 대기.
